...
/// </summary>
public sealed class SsdpAdvertiser : IAsyncDisposable
{
    ...
    readonly IPEndPoint _multicastEp = new(IPAddress.Parse(DlnaConstants.SsdpMulticastAddr), DlnaConstants.SsdpPort);

    UdpClient? _socket;
    Task? _responderTask;
    Task? _heartbeatTask;

    public SsdpAdvertiser(IPAddress bindAddr, int httpPort, string udn, LogSink log)
    {
        _bindAddr = bindAddr;
        _httpPort = httpPort;
        _udn = udn;
    ...
    public void Start()
    {
        ...
        {
            _log.Warn("SSDP", $"绑定 1900 端口失败（可能被占用）：{ex.Message}；M-SEARCH 将收不到。");
        }
        ...
        {
            _log.Warn("SSDP", $"加入多播组失败：{ex.Message}");
        }
        ...
        _log.Info("SSDP", $"Advertiser started on {_bindAddr}; Location={LocationUrl}; UDN=uuid:{_udn}");

        // 首发 alive
        _ = SendAliveAsync();
    }

    async Task ResponderLoopAsync(CancellationToken ct)
    {
        var sock = _socket!;
        while (!ct.IsCancellationRequested)
        {
            ...
            {
                _log.Warn("SSDP", $"接收失败：{ex.Message}");
    ...
    async Task HeartbeatLoopAsync(CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            {
                _log.Warn("SSDP", $"heartbeat error: {ex.Message}");
            }
        }
    }
    ...
    public async Task SendAliveAsync()
    {
        ...
        {
            var body = BuildNotify(usn, nt, alive: true);
    ...
    public async Task SendByeByeAsync()
    {
        foreach (var (usn, nt) in AllTargets())
        {
            var body = BuildNotify(usn, nt, alive: false);
            await SendToMulticastAsync(body).ConfigureAwait(false);
        }
    }

    async Task SendToMulticastAsync(string body)
    {
        ...
        {
            _log.Warn("SSDP", $"multicast send failed: {ex.Message}");
        }
    }

    async Task SendSearchResponseAsync(IPEndPoint to, string usn, string st)
    {
        if (_socket is null) return;
        var body =
            "HTTP/1.1 200 OK\r\n" +
        ...
        {
            _log.Warn("SSDP", $"M-SEARCH reply failed: {ex.Message}");
        }
    }

    string BuildNotify(string usn, string nt, bool alive) =>
        "NOTIFY * HTTP/1.1\r\n" +
    ...
    IEnumerable<(string usn, string nt)> AllTargets()
    {
        ...
        yield return ($"uuid:{_udn}::{DlnaConstants.ServiceTypeConnectionManager}", DlnaConstants.ServiceTypeConnectionManager);
        yield return ($"uuid:{_udn}::{DlnaConstants.ServiceTypeRenderingControl}", DlnaConstants.ServiceTypeRenderingControl);
    }

    IEnumerable<(string usn, string nt)> MatchedTargets(string st)
    {
        if (st == "ssdp:all")
        {
    ...
    bool ShouldRespondTo(string st)
    {
    ...
    {
        try { await SendByeByeAsync().ConfigureAwait(false); } catch { }
        _cts.Cancel();
        try { if (_responderTask is not null) await _responderTask.ConfigureAwait(false); } catch { }
