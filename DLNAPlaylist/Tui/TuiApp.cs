...
public sealed class TuiApp
{
    ...
    IReadOnlyList<RemoteDevice> _devices = Array.Empty<RemoteDevice>();
    IReadOnlyList<PendingApproval> _pending = Array.Empty<PendingApproval>();
    IReadOnlyList<LogEntry> _logTail = Array.Empty<LogEntry>();
    ...
    int _queueSel;
    int _deviceSel;
    FocusPane _focus = FocusPane.Queue;

    volatile bool _needPauseForPrompt;
    readonly SemaphoreSlim _promptGate = new(1, 1);

    enum FocusPane { Queue, Devices }
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        // 初始日志尾巴
        lock (_lock) _logTail = _log.Snapshot();
    ...
    async Task EventLoopAsync(ChannelReader<CoordinatorEvent> reader, CancellationToken ct)
    {
        try
        {
                ...
                lock (_lock)
                {
                    switch (e)
                    {
                        ...
                        case CoordinatorEvent.NowPlayingChanged x:
                            _nowPlaying = x.Item; _nowPos = x.Position; _nowDur = x.Duration; _isPaused = x.IsPaused;
                            break;
    ...
    void KeyLoop(CancellationToken ct)
    {
        while (!ct.IsCancellationRequested)
        {
            if (_needPauseForPrompt) { Thread.Sleep(80); continue; }
    ...
    void HandleKey(ConsoleKeyInfo k)
    {
        switch (k.Key)
        {
            case ConsoleKey.Q: _commands.TryWrite(new CoordinatorCommand.Stop()); Environment.Exit(0); break;
            case ConsoleKey.Tab:
            ...
            case ConsoleKey.UpArrow: MoveSel(-1, shift: (k.Modifiers & ConsoleModifiers.Shift) != 0); break;
            case ConsoleKey.DownArrow: MoveSel(+1, shift: (k.Modifiers & ConsoleModifiers.Shift) != 0); break;
    ...
    void MoveSel(int delta, bool shift)
    {
        lock (_lock)
        {
    ...
    static string BuildProgressBar(double pct, int width)
    {
        int full = (int)(pct * width);
        return "[" + new string('█', full) + new string('░', width - full) + "]";
    }

    IRenderable RenderLog()
    {
    ...
    async Task RunApprovalPromptAsync(CancellationToken ct)
    {
        await _promptGate.WaitAsync(ct).ConfigureAwait(false);
        try
        {
            PendingApproval? first;
            lock (_lock) first = _pending.FirstOrDefault();
            ...
            {
                case "允许（仅此一次）":
                    _commands.TryWrite(new CoordinatorCommand.ApproveCast(first.Id, RememberSource: false));
                    break;
                case "允许（并记住此来源）":
                    _commands.TryWrite(new CoordinatorCommand.ApproveCast(first.Id, RememberSource: true));
                    break;
                default:
                    _commands.TryWrite(new CoordinatorCommand.RejectCast(first.Id, BlockSource: false));
                    break;
            }
            AnsiConsole.Clear();
        }
